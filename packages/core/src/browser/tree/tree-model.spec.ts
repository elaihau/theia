...
import { ForwardTreeNodeIterator } from './tree-iterator';
...
describe('tree-model', () => {
    ...
    const findNode = (id: string) => model.getNode(id) as (SelectableTreeNode) || {
        id: 'missing',
        name: 'missing',
        selected: false,
        parent: undefined
    ...
    describe('selectRange', () => {
        ...
        it('should not affect the selection after selecting a range without `fromNode` and previous selection', () => {
            expectSelectionState();
            selectRange('1');

            // expectSelectionState();

            const itr = new ForwardTreeNodeIterator(findNode('1'));
            let next = itr.next();
            while (!next.done) {
                if (next.value) {
                    console.log('itr result', next.value!.id);
                } else {
                    console.log('skipped');
                }
                next = itr.next();
    ...
    describe('calculateRange', () => {
        ...
        ] as { from: string, to: string, expected: string[] }[]).forEach(test => {
            it(`should calculate the range between '${test.from}' and '${test.to}' => [${test.expected.join(', ')}]`, () => {
